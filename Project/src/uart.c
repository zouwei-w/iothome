...
int uart_dev_option(int *uart_fd, int speed)
{
    int ret;
    if((*uart_fd = uart_open("/dev/ttyUSB0")))
        fprintf(stdout,"ttyUSB0\n");
    else 
        return -1;

    ret = uart_init(*uart_fd, speed, 0, 8, 1, 'N');
    if(ret < 0)
    {
        fprintf(stderr, "Failed to uart_init");
        return -1;
    }
 ...
 * 出口参数： 正确返回为1，错误返回为0
 *******************************************************************/
int uart_open(char * serial_port)   // uart_open("/dev/ttyUSB0"); 
{
	int fd;
